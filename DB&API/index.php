...
switch ($method) {
    ...
    case 'INSERT_ADMIN':
        ...
        // Create SQL query for inserting Admin data
        $sql = "INSERT INTO place (id, name, descript, img, type) VALUES ('$id', '$name', '$descript', '$img', '$type')";

        if ($conn->query($sql) === TRUE) {
            $response = array('status' => 'success', 'message' => 'Admin data inserted successfully');
        } else {
            $response = array('status' => 'error', 'message' => 'Error inserting Admin data: ' . $conn->error);
        }
